...
## Fund vs Benchmark (Since Inception) Feature

This feature compares each fund's actual returns against its benchmark index returns since the first investment.

### How it works
1. **CAMS detection**: When a PDF is uploaded, the raw extracted text is scanned for "cams", "camsonline", or "kfintech" keywords. If found, `analysis.cas_source` is set to `"CAMS"`. Only CAMS reports show this section.
2. **Benchmark mapping**: `server/fund-benchmark.ts` loads three CSV files from `FundVsBenchmark)(sinceinception)/Right benchmark for right categories/` to build an ISIN → benchmark name map. A category-based fallback (by `fund_type`) is used when an ISIN has no explicit benchmark.
3. **Benchmark data**: Daily price history is loaded from CSV files in `Equity Benchmarks/`, `Debt Benchmarks/`, and `Hybrid Benchmarks/` subdirectories. Multiple year-split files (like Hybrid Arbitrage) are merged automatically.
4. **Comparison calculation**: For each SIP/PURCHASE transaction (date + amount), the benchmark price on that date is looked up and "benchmark units" are calculated. Current benchmark value = total units × latest price.
5. **API endpoint**: `GET /api/fund-vs-benchmark/:id` returns per-fund results including `fund_current_value`, `benchmark_current_value`, and `excess_return`.
6. **Frontend**: `client/src/components/FundVsBenchmark.tsx` renders a grouped bar chart (Invested / Fund Value / Benchmark Value) and a per-fund table showing Alpha and Beats/Lags status.